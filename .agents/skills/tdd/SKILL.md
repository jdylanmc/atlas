...
Use TypeScript, `node:test`, and `node:assert/strict` in `tests/*.test.ts`,
following existing fixtures and typed operation seams. Use the supported
Node.js 24.x and npm 11.6.2 toolchain. Start with
`node --test tests/<affected>.test.ts`; broaden only as required by the change.
`npm run ci` is the complete repository gate, including 100% product coverage.
Do not weaken its thresholds or add model/network dependencies to the SDK.

Durable review findings require an accept or reject case in
`tests/adversarial/`. Prefer data-only additions to an existing gate, following
that directory's README. Unit fixtures do not substitute for an installed
consumer or Git-backed integration proof when the requirement calls for one.
Defer nonessential refactoring to review; after green, limit cleanup to the
smallest behavior-preserving change needed by the current slice.
