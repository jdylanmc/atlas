...
The **phase boundary** is the gap between two phases. Prefer context changes
there, carrying unresolved decisions and evidence pointers forward. Continue
directly by default; delegation still requires a suitable bounded task and
permission under the active harness.
...
**1. Can you continue in this session?** Prefer continuing when the next phase
needs this phase's reasoning as a **primary source** and fits the active
harness's context budget. Grilling → implementation is the standard case:
the implementation benefits from the reasoning, not just a summary.
...
**4. Does the task warrant a separate context?** Delegate only substantial,
bounded independent work when permitted by the active harness. A simple
lookup or review that fits a direct read stays here. Use a background agent
only while doing independent work, and do not repeat its investigation.
