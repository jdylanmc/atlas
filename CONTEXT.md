...
**Lint Stamp**:
A machine-readable attestation that exact Atlas content completed a full Lint with matching evidence: its authoritative identity is the validated content's digest, while its commit is a run-specific review locator, so Atlas path or byte changes invalidate that identity but incidental Git metadata does not. Its trusted deterministic portion is reproducible validation evidence rather than committed Atlas knowledge; semantic verdicts are attested rather than reproduced or identified, the producing Atlas SDK version is not recorded, and schema, check, and Policy revisions belong only when authoritative revision sources exist.
