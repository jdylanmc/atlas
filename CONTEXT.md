...
**Core Archetype**:
One of the page kinds Atlas SDK owns in every Atlas: Anchor, Concept, Source, Principle, and Edge. An Atlas Schema extends a Core Archetype but never redefines what it means.
...
**Vocabulary Binding**:
The declared correspondence between one term in this glossary and the identifiers Atlas SDK contracts derive from that term. A binding makes a disagreement between the glossary and the product a reported Finding rather than an unnoticed rename, and an avoided term may never be bound.
