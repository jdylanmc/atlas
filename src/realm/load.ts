...
function containsUnsafeSchemaRegex(value: unknown): boolean {
  ...
  return Object.entries(value).some(
    ([key, item]) =>
      ...
      key === "$ref" ||
      containsUnsafeSchemaRegex(item),
  );
}
...
function parseManifest(
  ...
  validateKeys(realm, ["id", "title"], path, text, findings, "Manifest realm");
...
function realmFieldLocation(page: RealmPage, key: string): SourceLocation {
  const realmIndex = page.sourceLines.findIndex((line) => hasYamlKey(line, "realm"));
  const realmIndent = (
    (page.sourceLines[realmIndex] as string).match(/^\s*/) as RegExpMatchArray
  )[0].length;
