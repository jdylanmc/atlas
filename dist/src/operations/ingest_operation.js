...
import { addReceipt, canContinue, changeSetDigest, hasControlCharacter, isSafeGitBranchName as isSafeGitBranchNameShared, receiptFor, revisionDigest, } from "./operation_support.js";
...
function isSourceAuthority(value) {
    return value === "official" || value === "first-party" || value === "community"
        ? true
        : value === "opinion";
}
const ingestOperation = Object.freeze({
    kind: "ingest",
...
function finding(code, message, path = ".atlas", severity = "error") {
...
function pendingDecisions(findings) {
    const decisions = [];
    for (const entry of findings) {
        if (entry.severity !== "inconclusive")
            continue;
        if (entry.code === "ATLAS_INGEST_SCOPE_EXPANSION_PENDING") {
            decisions.push("A human must approve expanding the Ingest Scope before the crawled candidate outside it can be ingested.");
        }
        else {
            decisions.push("A human must adjudicate an equal-authority conflict Source Revision Time cannot settle.");
        }
    }
    return Object.freeze([...new Set(decisions)]);
}
function handoff(state, disposition, completion, findings, summary, evidence) {
    const decisions = pendingDecisions(findings);
    const staleWarnings = findings.filter((entry) => entry.code === "ATLAS_INGEST_SOURCE_STALE");
    return Object.freeze({
        ...
        degradationState: Object.freeze({
            reason: staleWarnings.length === 0
                ? evidence
                : `${evidence} Stale Knowledge remains traversable and its Source can be re-Ingested.`,
            state: staleWarnings.length === 0 ? "not-degraded" : "degraded",
        }),
        ...
        proposedChanges: completion === "completed"
            ...
            : Object.freeze({ reason: summary, state: "unknown" }),
        ...
        unresolvedHumanDecisions: decisions.length > 0
            ? Object.freeze({
            ...
            : Object.freeze({
                state: "none",
                summary: staleWarnings.length === 0
...
function result(state, completion, disposition, payload, findings, summary, evidence) {
    return Object.freeze({
...
function isCanonicalLocator(locator) {
    ...
    if (hasControlCharacter(locator))
        return false;
...
function trimWin32Segment(segment) {
...
function withinScope(locator, scope) {
    const included = [scope.entryPoint, ...scope.includedPaths];
    if (!included.some((prefix) => isPrefixPath(prefix, locator)))
        return false;
    if (scope.excludedPaths.some((prefix) => isPrefixPath(prefix, locator)))
        return false;
    return pathSegments(locator).length <= scope.maxDepth;
}
function slugForId(id, prefix) {
...
function activePrincipleTruthIds(content) {
    const ids = [];
    let active = false;
    for (const line of content.split(/\r?\n/u)) {
        if (/^## /u.test(line))
            active = line.trim() === "## Active truths";
        if (!active)
            continue;
...
function readExistingAtlas(files) {
    const decoder = new TextDecoder();
    const principleTruthIds = new Set();
    const atlasPolicyIds = new Set();
    const sourceText = new Map();
    const nonSourcePageIds = new Set();
...
function resolvedSources(graph) {
...
function unorderedPairKey(from, to) {
...
function collisionFindings(graph, scope, existing) {
    const findings = [];
    const collision = (id, directory, prefix, refreshable) => {
        ...
        if (slug === undefined)
            return;
        if (!existing.pageIds.has(id))
            return;
        if (refreshable && id === scope.sourceId)
            return;
        findings.push(finding("ATLAS_INGEST_ID_COLLISION", "A crawled page identity already names a page in the Home Atlas and would replace it; only a Source Refresh of that Source may reuse an identity.", `.atlas/${directory}/${slug}.md`));
    };
    for (const source of graph.sources) {
...
function validateSources(graph, scope) {
    const findings = [];
    const seen = new Set();
    for (const source of graph.sources) {
        const path = `.atlas/sources/${slugForId(source.id, "source") ?? "unknown"}.md`;
        if (slugForId(source.id, "source") === undefined) {
            findings.push(finding("ATLAS_INGEST_SOURCE_ID_INVALID", "Every ingested Source must carry a canonical source page identity.", path));
            continue;
        }
        if (seen.has(source.id)) {
            findings.push(finding("ATLAS_INGEST_SOURCE_ID_DUPLICATE", "Each ingested Source identity must appear once in the Candidate Graph.", path));
        }
        seen.add(source.id);
        if (source.content.trim() === "") {
            findings.push(finding("ATLAS_INGEST_SOURCE_CONTENT_REQUIRED", "A Source revision must carry the captured content its digest and Citations bind to.", path));
        }
        if (Number.isNaN(Date.parse(source.revisionTime))) {
            findings.push(finding("ATLAS_INGEST_SOURCE_REVISION_TIME_INVALID", "A Source Revision Time must be a comparable date-time asserted by the cited revision.", path));
        }
        if (!isCanonicalLocator(source.locator)) {
            findings.push(finding("ATLAS_INGEST_LOCATOR_INVALID", "A crawled locator must be a canonical repository-relative path.", path));
        }
        else if (!withinScope(source.locator, scope)) {
            findings.push(finding("ATLAS_INGEST_SCOPE_EXPANSION_PENDING", "A crawled Source lies beyond the approved Ingest Scope and pauses for approval.", path, "inconclusive"));
        }
        ...
        if (isSourceAuthority(scope.authority) &&
            ...
            findings.push(finding("ATLAS_INGEST_SOURCE_AUTHORITY_EXCEEDS_SCOPE", "A crawled Source claims a higher Source Authority than the approved Ingest Scope allows.", path));
        }
    }
    return findings;
}
...
function citationSupported(citation, sources, existing) {
    const candidate = sources.get(citation.sourceId);
    const existingText = existing.sourceText.get(citation.sourceId);
    if (candidate === undefined && existingText === undefined)
        return "missing-source";
    const span = citation.sourceClaim.trim();
    if (span === "")
        return "unsupported";
    if (span.length < minimumCitationSpanLength)
        return "span-too-short";
    const content = candidate === undefined ? existingText : candidate.content;
    return content.includes(citation.sourceClaim) ? "supported" : "unsupported";
}
function validateCitation(citation, path, sources, existing, findings) {
    const support = citationSupported(citation, sources, existing);
    if (support === "span-too-short") {
        findings.push(finding("ATLAS_INGEST_CITATION_SPAN_TOO_SHORT", "A Citation must quote a span long enough to bind its claim to the cited Source revision, not an incidental character.", path));
        return;
    }
    if (support === "missing-source") {
        findings.push(finding("ATLAS_INGEST_CITATION_SOURCE_MISSING", "A Citation must resolve to an Atlas-local Source in this Candidate Graph or the Home Atlas.", path));
        return;
    }
    if (support === "unsupported") {
        findings.push(finding("ATLAS_INGEST_CITATION_UNSUPPORTED", "A Citation must quote a claim that appears in the cited Source revision content.", path));
    }
}
function validateConcepts(graph, scope, sources, existing, reachable) {
    const findings = [];
    const seen = new Set();
    for (const concept of graph.concepts) {
        ...
        if (slug === undefined) {
            findings.push(finding("ATLAS_INGEST_CONCEPT_ID_INVALID", "Every Concept must carry a canonical concept page identity.", path));
            continue;
        }
        if (seen.has(concept.id)) {
            findings.push(finding("ATLAS_INGEST_CONCEPT_ID_DUPLICATE", "Each Concept identity must appear once in the Candidate Graph.", path));
        }
        seen.add(concept.id);
        if (concept.claim.trim() === "") {
            findings.push(finding("ATLAS_INGEST_CONCEPT_CLAIM_REQUIRED", "A Concept represents exactly one concept and must state its one claim.", path));
        }
        if (concept.citations.length === 0) {
            findings.push(finding("ATLAS_INGEST_CONCEPT_UNCITED", "A Concept must trace its claim to at least one Atlas-local Source through a Citation.", path));
        }
        ...
        if (!reachable.has(concept.id)) {
            findings.push(finding("ATLAS_INGEST_CONCEPT_UNREACHABLE", "A Concept must carry at least one Edge so no page is unreachable by traversal.", path));
        }
        if (!isCanonicalLocator(concept.locator)) {
            findings.push(finding("ATLAS_INGEST_LOCATOR_INVALID", "A crawled locator must be a canonical repository-relative path.", path));
        }
        else if (!withinScope(concept.locator, scope)) {
            findings.push(finding("ATLAS_INGEST_SCOPE_EXPANSION_PENDING", "A crawled Concept lies beyond the approved Ingest Scope and pauses for approval.", path, "inconclusive"));
        }
        findings.push(...validateContradiction(concept, path, existing));
    }
    return findings;
}
function validateContradiction(concept, path, existing) {
    ...
    if ((truthId === undefined) === (policyId === undefined)) {
        return [
            finding("ATLAS_INGEST_CONTRADICTION_UNRESOLVED", "An accepted Contradiction must name exactly one governor: an active Principle truth or an Atlas Policy in the Home Atlas.", path),
        ];
    }
    const governed = truthId !== undefined
        ? existing.principleTruthIds.has(truthId)
        : existing.atlasPolicyIds.has(policyId);
    if (!governed) {
        return [
            finding("ATLAS_INGEST_CONTRADICTION_UNRESOLVED", "An accepted Contradiction must name an active Principle truth or an Atlas Policy in the Home Atlas.", path),
        ];
    }
    if ((contradiction.acceptedBy ?? "").trim() === "") {
        return [
            finding("ATLAS_INGEST_CONTRADICTION_UNACCEPTED", "A Concept contradicting an active Principle truth or Atlas Policy cannot be ingested until a human accepts the Contradiction.", path),
        ];
    }
    return [];
}
function conceptById(graph) {
    return new Map(graph.concepts.map((concept) => [concept.id, concept]));
}
function primarySourceOf(concept, sources) {
    const citation = concept.citations[0];
    return citation === undefined ? undefined : sources.get(citation.sourceId);
}
function validateDisputes(graph, sources) {
    const findings = [];
    ...
    for (const dispute of graph.disputes) {
        ...
        if (left === undefined || right === undefined) {
            findings.push(finding("ATLAS_INGEST_DISPUTE_CONCEPT_MISSING", "A Dispute must name two Concepts present in this Candidate Graph."));
            continue;
        }
        ...
        if (leftSource === undefined || rightSource === undefined) {
            findings.push(finding("ATLAS_INGEST_DISPUTE_CONCEPT_MISSING", "A disputed Concept must cite a resolvable Source so its authority can settle the conflict."));
            continue;
        }
        ...
        if (Number.isNaN(leftTime) || Number.isNaN(rightTime) || leftTime === rightTime) {
            findings.push(finding("ATLAS_INGEST_DISPUTE_UNRESOLVED", "Equal-authority Concepts whose Source Revision Time cannot break the tie require human adjudication.", ".atlas", "inconclusive"));
        }
    }
    return findings;
}
function staleFindings(graph, scope) {
    const findings = [];
    ...
    for (const source of graph.sources) {
        ...
        const effectiveWindow = Math.min(source.refreshWindowDays, scope.freshnessWindowDays);
        if (elapsedDays > effectiveWindow) {
            findings.push(finding("ATLAS_INGEST_SOURCE_STALE", "The cited Source revision is older than its refresh window; its Stale Knowledge can be re-Ingested.", `.atlas/sources/${slugForId(source.id, "source") ?? "unknown"}.md`, "warning"));
        }
    }
    return findings;
}
function validateEdges(graph, scope, sources, existing) {
    const findings = [];
    const seen = new Set();
    ...
    const pairs = new Set(existing.edgePairs);
    ...
    for (const edge of graph.edges) {
        ...
        if (slug === undefined) {
            findings.push(finding("ATLAS_INGEST_EDGE_ID_INVALID", "Every Edge must carry a canonical edge page identity.", path));
            continue;
        }
        if (seen.has(edge.id)) {
            findings.push(finding("ATLAS_INGEST_EDGE_ID_DUPLICATE", "Each Edge identity must appear once in the Candidate Graph.", path));
        }
        seen.add(edge.id);
        if (edge.semantics.length === 0) {
            findings.push(finding("ATLAS_INGEST_EDGE_SEMANTICS_REQUIRED", "An Edge must assert at least one typed relationship semantic.", path));
        }
        for (const endpoint of [edge.from, edge.to]) {
            if (sourceIds.has(endpoint) || existing.sourceText.has(endpoint)) {
                findings.push(finding("ATLAS_INGEST_EDGE_CONNECTS_SOURCE", "Edges traverse Concepts, Anchors, and Principles; they never connect to a Source.", path));
            }
            else if (!conceptIds.has(endpoint) &&
                !existing.nonSourcePageIds.has(endpoint)) {
                findings.push(finding("ATLAS_INGEST_EDGE_ENDPOINT_MISSING", "An Edge endpoint must resolve to a page in this Candidate Graph or the Home Atlas.", path));
            }
        }
        if (edge.from === edge.to) {
            findings.push(finding("ATLAS_INGEST_EDGE_SELF_LOOP", "An Edge must relate two distinct pages.", path));
        }
        else {
            const key = unorderedPairKey(edge.from, edge.to);
            if (pairs.has(key)) {
                findings.push(finding("ATLAS_INGEST_EDGE_PAIR_DUPLICATE", "At most one Edge may exist per unordered in-Atlas page pair.", path));
            }
            pairs.add(key);
        }
        if (edge.citations.length === 0) {
            findings.push(finding("ATLAS_INGEST_EDGE_UNCITED", "An Edge must support its asserted relationship with at least one Citation.", path));
        }
...
function reachableEndpoints(graph, existing) {
    const conceptIds = new Set(graph.concepts.map((concept) => concept.id));
    const sourceIds = new Set(graph.sources.map((source) => source.id));
    const isSource = (endpoint) => sourceIds.has(endpoint) || existing.sourceText.has(endpoint);
    const isPage = (endpoint) => conceptIds.has(endpoint) || existing.nonSourcePageIds.has(endpoint);
    const reachable = new Set();
    for (const edge of graph.edges) {
        if (slugForId(edge.id, "edge") === undefined)
            continue;
        if (edge.from === edge.to)
            continue;
        const endpoints = [edge.from, edge.to];
        if (endpoints.some((endpoint) => isSource(endpoint)))
            continue;
        if (!endpoints.every((endpoint) => isPage(endpoint)))
            continue;
...
function validateApproval(scope) {
    if (scope.approvedBy.trim() !== "" && scope.approvedAt.trim() !== "") {
        return [];
    }
    return [
        finding("ATLAS_INGEST_APPROVAL_REQUIRED", "Ingest requires explicit Maintainer approval identity and time before it derives knowledge within the approved Ingest Scope."),
    ];
}
export function validateCandidateGraph(request, existingFiles) {
    ...
    if (!isSourceAuthority(scope.authority)) {
        findings.push(finding("ATLAS_INGEST_SCOPE_AUTHORITY_INVALID", "An Ingest Scope must assign a recognized Source Authority class."));
    }
    if (graph.sources.length === 0) {
        findings.push(finding("ATLAS_INGEST_SOURCE_REQUIRED", "Ingesting one repository source must record at least one Source."));
    }
    ...
    return Object.freeze(findings.toSorted((left, right) => {
...
function sdkMetadata(id, type, title, asOf, operationId) {
    return {
        "atlas-sdk-schema": "1.0.0",
        "created-at": asOf,
        "created-by": { kind: "agent", name: "Atlas SDK" },
        id,
        ...
        type,
        "updated-at": asOf,
        "updated-by": { kind: "agent", name: "Atlas SDK" },
    };
}
function pageEnvelope(path, sdk, atlas, body) {
    return {
        page: { atlas, body, sdk },
        source: {
...
function citationBody(citations) {
    const markers = citations.map((_, index) => `[^s${String(index + 1)}]`).join("");
    const definitions = citations
        .map((citation, index) => `[^s${String(index + 1)}]: [[.atlas/sources/${slugForId(citation.sourceId, "source")}]] Quoted span ${JSON.stringify(citation.sourceClaim.trim())}.`)
        .join("\n");
    return { definitions, markers };
}
function sourcePage(source, scope, operationId) {
    const slug = slugForId(source.id, "source");
    const atlas = {
        authority: source.authority,
    ...
    return pageEnvelope(`.atlas/sources/${slug}.md`, sdkMetadata(source.id, "source", source.title, scope.asOf, operationId), atlas, body);
}
function conceptPage(concept, scope, operationId) {
    const slug = slugForId(concept.id, "concept");
    const evidence = concept.citations.map((citation) => `.atlas/sources/${slugForId(citation.sourceId, "source")}`);
    const contradiction = concept.contradiction;
    ...
    return pageEnvelope(`.atlas/concepts/${slug}.md`, sdkMetadata(concept.id, "concept", concept.title, scope.asOf, operationId), atlas, bodyLines.join("\n"));
}
function edgePage(edge, scope, operationId) {
    const slug = slugForId(edge.id, "edge");
    const atlas = {
        from: edge.from,
    ...
    return pageEnvelope(`.atlas/edges/${slug}.md`, sdkMetadata(edge.id, "edge", edge.title, scope.asOf, operationId), atlas, body);
}
function changelogEntry(graph, scope, operationId, refresh) {
...
function structuredCitationPaths(citations) {
    return Object.freeze([
        ...new Set(citations.map((citation) => `.atlas/sources/${slugForId(citation.sourceId, "source")}.md`)),
 ...
 */
export function reconcileCandidateGraph(state, request, existingFiles = []) {
    const { candidateGraph: graph, scope } = request;
    const existing = readExistingAtlas(existingFiles);
    ...
    const changes = [
        ...
        ...serializeAtlasPages(pages).map((file) => Object.freeze({ content: file.content, path: file.path })),
    ];
    return Object.freeze({
        baseSnapshotDigest: state.baseSnapshotDigest,
        changes: Object.freeze(changes.toSorted((left, right) => compareCodePoints(left.path, right.path))),
...
export function validateCitationCorrespondence(request, changeSet) {
    const findings = [];
    const byPath = new Map(changeSet.changes.map((change) => [change.path, change]));
    const check = (id, directory, prefix, citations) => {
        const slug = slugForId(id, prefix);
        if (slug === undefined)
            return;
        ...
        const equal = bodyPaths.length === expected.length &&
            bodyPaths.every((value, index) => value === expected[index]);
        if (!equal) {
            findings.push(finding("ATLAS_INGEST_CITATION_CORRESPONDENCE", "A derived page's body cites Sources its validated Citations do not; every emitted Citation must correspond to a structured Citation.", path));
        }
    };
...
export function validateAtlasIngestChangeSet(state, changeSet) {
    const findings = [];
    if (changeSet.targetHead !== state.targetHead ||
        changeSet.baseSnapshotDigest !== state.baseSnapshotDigest) {
        findings.push(finding("ATLAS_INGEST_CHANGE_SET_STALE", "Ingest Atlas Change Set base does not match the current base snapshot."));
    }
    for (const change of changeSet.changes) {
        if (!change.path.startsWith(".atlas/") ||
            ...
            findings.push(finding("ATLAS_INGEST_CHANGE_SET_PATH_INVALID", "Ingest Atlas Change Sets may write only canonical .atlas paths.", change.path));
        }
    }
    const changelog = changeSet.changes.find((change) => change.path === ".atlas/CHANGELOG.md");
    if (changelog === undefined) {
        findings.push(finding("ATLAS_INGEST_CHANGELOG_REQUIRED", "A successful Ingest proposal must append an operation-identified Atlas Changelog entry."));
    }
    else if (!changelog.content.includes(state.operationId)) {
        findings.push(finding("ATLAS_INGEST_CHANGELOG_OPERATION_ID_REQUIRED", "The Atlas Changelog entry for an Ingest proposal must name the stable operation ID.", ".atlas/CHANGELOG.md"));
    }
    return Object.freeze(findings);
}
function validateResumeReceipts(state, changeSet) {
    ...
    if (writeReceipt !== undefined &&
        ...
        findings.push(finding("ATLAS_INGEST_RESUME_CHANGE_SET_MISMATCH", "Ingest resume receipts must be content-addressed to the current Atlas Change Set digest and written tree."));
    }
    ...
    if (commitReceipt !== undefined &&
        ...
        findings.push(finding("ATLAS_INGEST_RESUME_CHANGE_SET_MISMATCH", "Ingest commit receipts must bind the current Atlas Change Set digest to the proposal commit."));
    }
    const lintReceipt = receiptFor(state, "lint-proposal");
    if (lintReceipt !== undefined &&
        (lintReceipt.changeSetDigest !== digest ||
            lintReceipt.lintEvidenceCommit !== lintReceipt.receipt)) {
        findings.push(finding("ATLAS_INGEST_RESUME_CHANGE_SET_MISMATCH", "Ingest Lint receipts must bind the current Atlas Change Set digest to the lint evidence commit."));
    }
    if (commitReceipt !== undefined &&
        lintReceipt !== undefined &&
        (lintReceipt.lintEvidenceCommit ?? lintReceipt.receipt) !==
            (commitReceipt.commit ?? commitReceipt.receipt)) {
        findings.push(finding("ATLAS_INGEST_LINT_STAMP_STALE", "Ingest refused Lint evidence for a proposal commit different from the committed Atlas Change Set."));
    }
    return Object.freeze(findings);
}
const evidenceSummary = (request) => `Ingest of ${request.scope.sourceId} recorded ${String(request.candidateGraph.sources.length)} Source(s), ${String(request.candidateGraph.concepts.length)} Concept(s), and ${String(request.candidateGraph.edges.length)} Edge(s) within the approved Ingest Scope.`;
export function runAtlasIngestWorkflow(state, request, runtime) {
    ...
    try {
        if (!isSafeGitBranchName(state.proposalBranch) ||
            ...
            return result(state, "not-completed", "failed", {}, Object.freeze([
                finding("ATLAS_INGEST_WORKFLOW_STATE_INVALID", "Ingest workflow state names an unsafe branch."),
            ]), "Ingest refused unsafe workflow state before mutating.", evidence);
        }
        if (runtime.currentTargetHead() !== state.targetHead ||
            runtime.currentBaseSnapshotDigest() !== state.baseSnapshotDigest) {
            return result(state, "not-completed", "failed", {}, Object.freeze([
                finding("ATLAS_INGEST_BASE_SNAPSHOT_STALE", "Ingest refused stale mutation because the target branch or base snapshot digest changed."),
            ]), "Ingest requires a refreshed base snapshot before mutating.", evidence);
        }
        ...
        if (emittable) {
            try {
                ...
                emissionFindings = Object.freeze([
                    finding("ATLAS_INGEST_PAGE_EMISSION_INVALID", "A derived page could not be emitted through the house serializer, so nothing was written."),
                ]);
            }
        ...
        if (!canContinue(findings) || changeSet === undefined) {
            return result(state, "not-completed", "failed", changeSet === undefined ? {} : { changeSet }, findings, pendingDecisions(findings).length > 0
                ? "Ingest paused for unresolved human decisions before mutating."
                : "Ingest is blocked by Candidate Graph validation Findings.", evidence);
        }
        let nextState = state;
        if (receiptFor(nextState, "create-proposal-worktree") === undefined &&
            runtime.workspaceExists?.() === true) {
            return result(nextState, "not-completed", "failed", {}, Object.freeze([
                finding("ATLAS_INGEST_WORKSPACE_EXISTS", "Ingest found an existing proposal branch or Operation Workspace before creating a new proposal."),
            ]), "Ingest refused to overwrite an existing Operation Workspace.", evidence);
        }
        if (receiptFor(nextState, "create-proposal-worktree") === undefined &&
            runtime.workspacePathValid?.() === false) {
            return result(nextState, "not-completed", "failed", {}, Object.freeze([
                finding("ATLAS_INGEST_WORKSPACE_PATH_INVALID", "Ingest refused an Operation Workspace path that escapes the Atlas Host Directory."),
            ]), "Ingest refused to create an Operation Workspace outside the Atlas Host Directory.", evidence);
        }
        const digest = changeSetDigest(changeSet);
        if (receiptFor(nextState, "create-proposal-worktree") === undefined) {
        ...
        if (linted.lint.completion !== "completed" ||
            linted.lint.disposition !== "success") {
            return result(nextState, "not-completed", "failed", { changeSet, lint: linted.lint }, linted.lint.handoff.validationState.findings, "Ingest proposal did not pass trusted Lint.", evidence);
        }
        if (linted.receipt !== commit) {
            return result(nextState, "not-completed", "failed", { changeSet, lint: linted.lint }, Object.freeze([
                finding("ATLAS_INGEST_LINT_STAMP_STALE", "Ingest refused to stamp a proposal commit different from the Lint evidence commit."),
            ]), "Ingest refused a stale Lint Stamp.", evidence);
        }
        return result(nextState, "completed", "success", { changeSet, lint: linted.lint }, staleFindings(request.candidateGraph, request.scope), "Ingest produced a Linted Atlas Proposal with cited Source and derived knowledge.", evidence);
    }
    catch {
        return result(latestState, "not-completed", "failed", {}, Object.freeze([
            finding("ATLAS_INGEST_RUNTIME_FAILED", "Ingest runtime failed before the operation completed."),
        ]), "Ingest did not complete.", evidence);
    }
}
