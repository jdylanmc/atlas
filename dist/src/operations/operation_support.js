...
import { sha256Hex } from "../atlas/sha256.js";
 ...
 */
export function isSafeGitBranchName(name) {
 ...
 */
export function revisionDigest(content) {
 ...
 */
export function changeSetDigest(changeSet) {
    const parts = [];
    const frame = (field) => {
    ...
    const ordered = [...changeSet.changes].toSorted((left, right) => compareCodePoints(left.path, right.path));
...
export function receiptFor(state, effect) {
...
export function canContinue(findings) {
    return findings.every((entry) => entry.severity !== "error" && entry.severity !== "inconclusive");
}
