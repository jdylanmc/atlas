...
function assertRepresentable(value, path) {
    if (value === null || typeof value !== "object") {
        return;
    }
    const isArray = Array.isArray(value);
    for (const key of Reflect.ownKeys(value)) {
        const kept = typeof key === "string" &&
            (isArray
...
export function serializeAtlasPages(pages) {
    const ordered = pages.toSorted((left, right) => compareCodePoints(left.source.path, right.source.path));
    ...
    const files = ordered.map((parsed) => Object.freeze({
