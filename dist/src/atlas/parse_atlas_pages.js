...
import { checkAtlasPageEnvelope, } from "../domain/atlas_page.js";
import { corePageDirectories } from "../domain/core_archetype.js";
...
export class AtlasPageParseError extends Error {
    ...
    path;
    sourceLine;
    constructor(code, path, sourceLine) {
...
function openingLengthOf(content) {
 ...
 */
export function atlasFrontmatterSpan(content) {
...
function frontmatterDepthBound(frontmatter) {
    let bound = 0;
    let flowDepth = 0;
...
function bodyEndLine(body, startLine) {
    if (body === "") {
        return startLine;
    }
...
export function classifyAtlasTextPath(path) {
    ...
    return match !== null && corePageDirectories.has(match[1])
        ? "page"
        : "opaque";
}
...
function findClosingDelimiter(content, from) {
    let index = from;
 ...
 */
export function parseAtlasPage(file) {
    ...
    const frontmatter = cloneAndFreezeJson(document.toJS({ mapAsMap: true, maxAliasCount: 0 }));
