...
import { initialAtlasInitializationWorkflowState, isSafeGitBranchName, notCompletedAtlasInitializationResult, runAtlasInitializationWorkflow, } from "../operations/initialize_operation.js";
import { runLintOperation } from "../operations/lint_operation.js";
import { captureLocalAtlasSnapshot } from "./local_atlas_snapshot.js";
import { runTrustedGit, runTrustedGitForWrite } from "./trusted_git.js";
...
function gitWrite(repository, args) {
...
function digestSnapshot(repository, targetHead) {
...
function proposalBranchName(targetHead) {
    return `atlas-initialization-${targetHead.slice(0, 12)}`;
}
function workspacePath(repository, proposalBranch) {
    return join(repository, ".atlas-operation-workspaces", proposalBranch);
}
function workspaceExists(repository, proposalBranch) {
    return (existsSync(workspacePath(repository, proposalBranch)) ||
...
function workspacePathIsContained(repository, proposalBranch) {
...
function statePath(repository, proposalBranch) {
    return join(workspacePath(repository, proposalBranch), ".atlas-operation-state.json");
}
function writeStateAtomically(repository, state) {
...
function gitCommonDirectory(repository) {
    const common = git(repository, ["rev-parse", "--git-common-dir"]);
    return resolve(repository, common);
}
function excludeOperationWorkspaces(repository) {
...
function parseWorkflowState(value) {
    ...
    const state = value;
    if (state["operation-workflow-schema"] !== "1.0.0" ||
    ...
    return Object.freeze({
        ...
        effectReceipts: Object.freeze(state.effectReceipts.map((receipt) => {
            ...
            if (effect !== "create-proposal-worktree" &&
...
export function createLocalAtlasInitializationState(repository) {
...
export function readLocalAtlasInitializationState(repository, proposalBranch) {
    const state = parseWorkflowState(JSON.parse(readFileSync(statePath(repository, proposalBranch), "utf8")));
    if (state.proposalBranch !== proposalBranch ||
        !/^atlas-initialization-[0-9a-f]{12}$/u.test(state.proposalBranch)) {
        throw new Error("operation state proposal branch is malformed");
    }
    return state;
}
export function resumeLocalAtlasInitialization(repository, proposalBranch) {
    if (!isSafeGitBranchName(proposalBranch)) {
        return notCompletedAtlasInitializationResult({
            ...
            recommendedNextAction: "Resume with a safe proposal branch name emitted by a prior Initialization result.",
            summary: "Initialization refused unsafe resume state before reading workspace state.",
        });
    }
    try {
        return runLocalAtlasInitialization(repository, readLocalAtlasInitializationState(repository, proposalBranch));
    }
    catch {
        return notCompletedAtlasInitializationResult({
            code: "ATLAS_INITIALIZATION_STATE_UNREADABLE",
            message: "Atlas Initialization could not read the persisted workflow state for the requested proposal branch.",
            recommendedNextAction: "Inspect the Operation Workspace state file before attempting resume again.",
            summary: "Initialization could not resume from persisted workflow state.",
        });
    }
}
export function runLocalAtlasInitialization(repository, state) {
    let workflowState;
    try {
        ...
        return notCompletedAtlasInitializationResult({
            ...
            message: "Atlas Initialization could not capture a Git-backed base snapshot for the Atlas Host Directory.",
            recommendedNextAction: "Run Initialization from a Git worktree with an existing commit, then retry.",
    ...
    return runAtlasInitializationWorkflow(workflowState, {
        ...
        currentBaseSnapshotDigest: () => digestSnapshot(repository, git(repository, ["rev-parse", workflowState.targetBranch])),
        lintProposal: () => {
        ...
        workspacePathValid: () => workspacePathIsContained(repository, workflowState.proposalBranch),
