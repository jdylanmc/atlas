import { loadAtlasText, AtlasLoadError, } from "../atlas/load_atlas_text.js";
import { rethrowProcessLimit } from "../atlas/process_limit.js";
import { validateAtlasStructureWithPages } from "./validate_atlas_structure.js";
const attribution = Object.freeze({
    checkId: "sdk-core.atlas-input",
    kind: "sdk-core",
    trusted: true,
});
const loadCodes = Object.freeze({
...
function loadFinding(code, message) {
...
function captureOnce(capturedFiles, budgets) {
    return {
        budgets: Object.freeze({
            maxFileBytes: budgets.maxFileBytes,
            maxTotalBytes: budgets.maxTotalBytes,
        }),
        files: Array.from(capturedFiles, (file) => Object.freeze({ bytes: file.bytes, path: file.path })),
    };
}
function loadOrFinding(capturedFiles, budgets) {
    let captured;
    try {
        ...
        return loadFinding("ATLAS_CAPTURE_UNREADABLE", "Captured Atlas files could not be read from the program running the Lint.");
    }
    try {
        ...
        return loadFinding("ATLAS_LOAD_FAILED", "Captured Atlas files could not be loaded.");
    }
}
const atlasInputValidationBrand = Symbol("atlas-input-validation");
const noFiles = Object.freeze([]);
const noPages = Object.freeze([]);
function validationState(findings) {
 ...
 */
export function loadAndValidateAtlasInput(capturedFiles, budgets) {
    const loaded = loadOrFinding(capturedFiles, budgets);
    if ("code" in loaded) {
        const findings = Object.freeze([loaded]);
        return Object.freeze({
            [atlasInputValidationBrand]: true,
            files: noFiles,
            findings,
            pages: noPages,
