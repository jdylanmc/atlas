  ...
  test(`adversarial installed-consumer corpus: ${entry.name}`, () => {
    ...
    try {
      ...
      if (entry.cacheFailure !== undefined) {
        if (
          ["invalid-lock-on-update", "lock-persistence"].includes(
        ...
        const probe = spawnSync(
          ...
          [
            ...
            [
              ...
              'for (const bytes of [\'{"dependencies":null}\', \'{"dependencies":[null]}\', \'{"dependencies":[\', "42", "null", "[]", \'{"dependencies":[{"cacheKey":" "}]}\' ]) {',
              ...
              'verifyExplore(updated, first.snapshot.snapshot, true, ["ATLAS_CROSS_ATLAS_LOCK_REPAIR_FAILED"]);',
              ...
              'console.log(JSON.stringify({ state: offline.state, code: "ATLAS_CROSS_ATLAS_LOCK_REPAIR_FAILED", offlineCode: offline.maintenanceFindings?.[0]?.code }));',
